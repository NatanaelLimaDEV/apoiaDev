...
export function FormDonate({ creatorId, slug }: FormDonateProps) {
    ...
    async function onSubmit(data: FormData) {
        const priceInCents = Number(data.price) * 100; // Converter para centavos
    ...
    async function handlePaymentResponse(checkout: { sessionId?: string, error?: string }) {
        ...
        if (!checkout.sessionId) {
            toast.error("Falha ao criar sessão de pagamento. Tente novamente mais tarde.")
            return
        }
    ...
    return (
        <Card className="shadow-xl border-0 bg-white/95 backdrop-blur-sm h-fit">
            <CardHeader>
                ...
                <CardDescription>
                    Sua contribuição ajudar a manter o contéudo
                </CardDescription>
            </CardHeader>
            <CardContent>
                <Form {...form}>
